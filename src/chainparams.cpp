 ...
 * Premine pubkeys
 */
std::string gPreminePubkeyHash = "af879a6ead7fc206fd368311c25dede96d4ae127";
...
public:
    CMainParams() {
        ...
        consensus.BTSQHeight = 506066; // Around 26/01/2018 00:00 UTC
        consensus.BTSQPremineWindow = 1;
        consensus.BTSQPremineAmount = 1260000;
        consensus.BTSQPremineEnforceWhitelist = true;
        ...
        nDefaultPort = 8866; // different port than Bitcoin
        ...
        vSeeds.emplace_back("dnsseed.aliyinke.com", true); 
        vSeeds.emplace_back("seed1.aliyinke.com", true); 
        vSeeds.emplace_back("seed2.aliyinke.com", true); 
        vSeeds.emplace_back("seed3.aliyinke.com", true); 

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,63);  // prefix: S
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,58);  // prefix: Q
        ...
        vPreminePubkeyHash = gPreminePubkeyHash;
    }
};
...
public:
    CRegTestParams() {
        ...
        consensus.BTSQHeight = 2000;
        consensus.BTSQPremineWindow = 1;
        consensus.BTSQPremineAmount = 126000;                
        consensus.BTSQPremineEnforceWhitelist = true;
...
CScript CPremineSigScript(const std::string pubkeyhash) {
    CScript scriptPubKeyHash = CScript() << OP_DUP << OP_HASH160 << ParseHex(pubkeyhash) << OP_EQUALVERIFY << OP_CHECKSIG;
    return scriptPubKeyHash;
}

bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    assert((uint32_t)consensus.BTSQHeight <= height &&
           height < (uint32_t)(consensus.BTSQHeight + consensus.BTSQPremineWindow));
    CScript scriptDummy = CScript() << OP_TRUE;
    if (!consensus.BTSQPremineEnforceWhitelist || scriptPubKey == scriptDummy) {
        return true;
    }
    CScript redeem_script = CPremineSigScript(vPreminePubkeyHash);
