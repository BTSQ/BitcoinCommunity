...
const std::string strMessageMagic = "Bitcoin Community Signed Message:\n";
...
bool static IsBTSQHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.BTSQHeight;
}

bool IsBTSQHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsBTSQHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsBTSQHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsBTSQHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.BTSQHeight;
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight >= consensusParams.BTSQHeight && nHeight < consensusParams.BTSQHeight + consensusParams.BTSQPremineWindow && consensusParams.BTSQPremineEnforceWhitelist)
        nSubsidy = consensusParams.BTSQPremineAmount / consensusParams.BTSQPremineWindow * COIN * COIN_SCALE;
    else
        nSubsidy >>= halvings;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsBTSQHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.BTSQHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after BTSQ fork.
    if (nHeight >= consensusParams.BTSQHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BTSQHeight &&
        nHeight < consensusParams.BTSQHeight + consensusParams.BTSQPremineWindow &&
        consensusParams.BTSQPremineEnforceWhitelist)
    {
